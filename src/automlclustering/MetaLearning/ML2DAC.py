...
class ML2DAC:
    def __init__(self, mkr_path="../automlclustering/MetaKnowledgeRepository",
                 shape_sets=None,  # Datasets from the Learning Phase
                 mf_set=MetaFeatureExtractor.meta_feature_sets[0],
                 k_range=None, random_state=1234):
        self.mkr_path = Path(mkr_path)
        self.mf_set = mf_set
        self.k_range = k_range
        self.mf_df = pd.read_csv(self.mkr_path / "meta_features" / f"{mf_set_to_string(self.mf_set)}_metafeatures.csv")

        self.random_state = random_state
        self.cvi_predictor = self._train_cvi_model()
    ...
    def find_similar_dataset(self, meta_features, dataset_name, n_similar_datasets=1):
        ...
        dataset_names_to_use = self.mf_df["dataset"].unique()
    ...
    def _remove_duplicates_from_ARI_s(ARI_s):
        print(ARI_s)
        ARI_s["algorithm"] = ML2DAC._assign_algorithm_column(ARI_s)
        ARI_s = ARI_s.drop_duplicates(subset="config", keep='first')
        ARI_s = ARI_s.drop_duplicates(subset=["algorithm", "AMI"], keep='first')
    ...
    def select_warmstart_configurations(ARI_s, n_warmstarts):
        if n_warmstarts > 0:
            ...
            print(ARI_s)
            warmstart_configs = ARI_s.sort_values("AMI", ascending=True).reset_index()[0:n_warmstarts]
    ...
    def define_config_space(self, warmstart_configs, limit_cs=True, X=None):
        if limit_cs:
            warmstart_configs["algorithm"] = ML2DAC._assign_algorithm_column(warmstart_configs)
            ...
            cs = ClusteringCS.build_config_space(k_range=self.k_range, X_shape=X.shape)
    ...
    def retrieve_ARI_values_for_similar_dataset(EC, D_s):
        ...
        ARI_s = EC_s[["config", "AMI"]]
        ARI_s = ML2DAC._remove_duplicates_from_ARI_s(ARI_s)
        return ARI_s
    ...
    def optimize_with_meta_learning(self, X,
        ...
        cs, algorithms = self.define_config_space(warmstart_configs, limit_cs, X)
    ...
    def _train_cvi_model(self):
        classifier = LearningPhase.train_classifier(mkr_path=self.mkr_path, random_state=self.random_state)
        return classifier
...
if __name__ == '__main__':
    ...
    X, _ = make_blobs()
    ml2dac = ML2DAC(mkr_path=Path("../MetaKnowledgeRepository"))
